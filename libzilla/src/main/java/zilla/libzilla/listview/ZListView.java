...
public class ZListView extends ListView implements OnScrollListener {
    ...
    private ZListViewFooter mFooterView;
    ...
    private void initWithContext(Context context, AttributeSet attrs) {
        headerHeight = getResources().getDimensionPixelSize(R.dimen.zlist_header_height);//default head height
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ZListViewProperties);
        int selector = typedArray.getResourceId(R.styleable.ZListViewProperties_item_selector,
                R.drawable.zlist_item_selector);
        ...
        mHeaderViewContent = (RelativeLayout) mHeaderView
                .findViewById(R.id.zlistview_header_content);
        mHeaderTimeView = (TextView) mHeaderView
                .findViewById(R.id.zlistview_header_time);
        addHeaderView(mHeaderView);
        mFooterView = new ZListViewFooter(context);
        mFooterViewContent = (RelativeLayout) mFooterView
                .findViewById(R.id.zlistview_footer_content);
    ...
    public void setPullLoadEnable(boolean enable) {
        ...
        if (!mEnablePullLoad) {
            ...
            mFooterViewContent.setVisibility(View.VISIBLE);
            mFooterView.setState(ZListViewFooter.STATE_NORMAL);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (mEnablePullRefresh && !mPullRefreshing) {
            if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
                mHeaderView.setState(ZListViewHeader.STATE_READY);
            } else {
                mHeaderView.setState(ZListViewHeader.STATE_NORMAL);
            }
        }
        setSelection(0);
    }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading) {
            if (height > PULL_LOAD_MORE_DELTA) {
                mFooterView.setState(ZListViewFooter.STATE_READY);
            } else {
                mFooterView.setState(ZListViewFooter.STATE_NORMAL);
            }
        }
    ...
    private void startLoadMore() {
        mPullLoading = true;
        mFooterView.setState(ZListViewFooter.STATE_LOADING);
            ...
            default:
                ...
                if (getFirstVisiblePosition() == 0) {
                    ...
                    if (mEnablePullRefresh
                        ...
                        mHeaderView.setState(ZListViewHeader.STATE_REFRESHING);
                        if (mListViewListener != null) {
    ...
    public void setHeaderTransparent() {
        mHeaderView.setTansparent();
    }

    /**
     * adjustSlideRightHeaderView
     */
    public void adjustSlideRightHeaderView() {
        int height = mHeaderView.getVisiableHeight();
        if (height == 0) {
            return;
        }
    ...
    public void showRefreshProgress() {
        mHeaderView.setVisiableHeight(headerHeight);
        mPullRefreshing = true;
        mHeaderView.setState(ZListViewHeader.STATE_REFRESHING);
    }

    public void setFootVisibility(boolean enable) {
        if (!enable) {
